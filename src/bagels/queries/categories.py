...
def get_all_categories_tree():  # special function to get the categories in a tree format
    with app.app_context():
        ...
        categories = (
            Category.query.options(db.joinedload(Category.parentCategory))
...
def get_all_categories_records(
    ...
    with app.app_context():
        ...
        base_query = (
            db.session.query(Category, db.func.sum(Record.amount).label("net_amount"))
        ...
        if subcategories:
            categories = (
                base_query.group_by(Category.id).order_by(db.desc("net_amount")).all()
            )
        else:
            categories = (
                base_query.group_by(Category.id).order_by(db.desc("net_amount")).all()
            ...
            for category, net_amount in categories:
                ...
                parent_category = (
                    category.parentCategory if category.parentCategory else category
                )
        ...
        for category, net_amount in categories:
            ...
            recent_records_query = (
                Record.query.filter(Record.categoryId == category.id)
                .filter(Record.date >= start_of_period)
                .filter(Record.date < end_of_period)
                .filter(Record.isIncome == (not isExpense))
            )
            ...
            recent_records = (
                recent_records_query.order_by(Record.date.desc())
                .limit(record_limit)
                .all()
            )
            ...
            category.percentage = (
                int((abs(net_amount or 0) / total_amount * 100))
                if total_amount > 0
                else 0
            )
