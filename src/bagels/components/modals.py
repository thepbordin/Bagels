    ...
    def compose(self) -> ComposeResult:
        yield ModalContainer(
            Container(
                ...
                Container(
                    Accounts(
                        self.accounts, initial_index=self.fromAccount - 1, type="from"
                    ),
                    Label(">>>", classes="arrow"),
                    Accounts(
                        self.accounts, initial_index=self.toAccount - 1, type="to"
                    ),
...
class RecordModal(InputModal):
    ...
    def action_add_split(self, paid: bool = False):
        ...
        new_split_form_fields = self.record_form.get_split_form(
            current_split_index, paid
        )
