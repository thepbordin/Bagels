...
must_haves:
  truths:
    ...
    - User can batch import records from YAML with `bagels add record --from-yaml`
  artifacts:
    - path: src/bagels/cli/records.py
      provides: Click command group for record queries
      exports: ["records", "list_records", "show_record", "add_record"]
      min_lines: 200
    - path: tests/cli/test_records.py
      provides: Integration tests for records commands
...
<objective>
Implement records query commands with comprehensive filtering capabilities, enabling users to list, show, and batch import financial records.

Purpose: Provide the primary interface for querying financial records with flexible filtering by month, category, date range, and amount. This is the most frequently used query command and serves as the pattern for other query commands.

Output: Working `bagels records` command group with list, show, and add subcommands.
</objective>
...
4. **add subcommand with YAML import** (implements CLI-10):
   ```python
   @records.command("add")
...
python -c "from bagels.cli.records import records; print(records.name)"
  ...
  <done>Records command group created with list/show/add subcommands</done>
</task>
...
<task type="auto">
  <name>Task 4: Implement batch import from YAML</name>
  <files>src/bagels/cli/records.py</files>
  <action>
...
3. **Import logic**:
   ...
   - Resolve foreign keys (accountSlug → accountId, etc.)
   - Generate slug for new records
...
<task type="auto">
  <name>Task 5: Register records command group</name>
  <files>src/bagels/__main__.py</files>
  <action>
Register records command group in main CLI:
...
3. **Verify help text**:
   ...
   - `bagels records --help` should show list/show/add subcommands

Follow existing command registration pattern (export, import, init).
  </action>
  <verify>
bagels --help | grep records
bagels records --help | grep "Query and manage records"
  </verify>
  <done>Records command group accessible via `bagels records`</done>
</task>

<task type="auto">
  <name>Task 6: Create integration tests for records</name>
  <files>tests/cli/test_records.py</files>
  <action>
Create comprehensive integration tests for records commands:
...
3. **Add command tests** (CLI-10):
   - test_add_from_yaml: Verify batch import from YAML file
   - test_add_from_yaml_validation: Verify validation errors display
   - test_add_from_yaml_foreign_keys: Verify FK resolution

Use CliRunner from click.testing. Use sample_db_with_records fixture.
  </action>
  <verify>
pytest tests/cli/test_records.py -x (all tests pass)
  </verify>
  <done>CLI-01, CLI-02, CLI-10 requirements met with test coverage</done>
</task>
...
6. `bagels records add --from-yaml data.yaml` imports records
</verification>
...
4. Add command imports records from YAML with validation
5. All commands support table/JSON/YAML output formats
6. Test coverage for all commands and filter combinations
</success_criteria>

<output>
After completion, create `.planning/phases/02-cli-query-layer/02-02-SUMMARY.md`
</output>
